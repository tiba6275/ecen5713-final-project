...
void sendImage(const std::string& filename, const std::string& serverIp, const std::string& port) {
    std::string pipelineStr = "filesrc location=" + filename + " ! jpegenc ! tcpclientsink host=" + serverIp + " port=" + port;
...
int main(int argc, char *argv[]) {
    ...
    sendImage(inFile1, serverIp, port);

    while (true) {
        captureImage(inFile2);
        sendImage(inFile2, serverIp, port);
        ...
        sendImage(outFile, serverIp, port);
